...
void preprocessFont()
{
	font::letter_width[0] = 14;
	int letters_count = sizeof(font::letters) / sizeof(std::vector<float>*);

	for (int letter = 1; letter < letters_count; letter++)
	{
		//min-max
		float min_x = 1000000; float max_x = -100000;
		for (int i = 0; i < font::letters[letter]->size(); i += 2)
		{
			float x = font::letters[letter]->at(i);
			min_x = min(min_x, x);
			max_x = max(max_x, x);
		}
		...
		for (int i = 0; i < font::letters[letter]->size(); i += 2)
		{
			font::letters[letter]->at(i) -= min_x;
		}

		//scale
		float scale = .1;

		for (int i = 0; i < font::letters[letter]->size(); i++)
		{
			font::letters[letter]->at(i) *= scale;
		}


		float width = max_x - min_x;

		font::letter_width[letter] = width * scale;
...
float drawLetter(int letter, float x, float y, float scale, bool getSize = false)
{
	...
	if (getSize)
	{
		return font::letter_width[letter] * scale;
	}
	...
	const std::vector<float>* pts = font::letters[letter];
