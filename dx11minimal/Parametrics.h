...
private:
    ...
    void generateStar(point3d& p) {
        ...
        if (rand() % 100 < 20) {// Ïðîöåíò Çâ¸çä â öåíòðå 
...
public:
    ...
    void draw() {
        ...
        for (int i = 0; i < 6000; i++) {
            ...
            float distance = sqrt(star.x * star.x + star.y * star.y);  
            float normalizedDist = min(distance / 300.0f, 1.0f);

            COLORREF starColor;


            if (distance < 100) {   // Öâåòà öåíòðà ðàíäîì äëÿ êðàñîòû
                int red = 255;
            ...
            else if (distance < 400) {  // Öâåòà Ñïèðàëåé òîæå ñ ðàíäîìîì
