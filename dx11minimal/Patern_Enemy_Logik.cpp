...
using namespace Enemy;
    ...
    void EnemyAI::AiUpdate(float deltaTime, float& player) {

        Transform* Ptransform = Player->GetComponent<Transform>();
        Transform* Etransform = Enemy->GetComponent<Transform>();

        data.playerPosHistory.push_back(Ptransform->position);
        if (data.playerPosHistory.size() > 30) {
            data.playerPosHistory.erase(data.playerPosHistory.begin());
        }

        data.playerDistance = (Ptransform->position - Etransform->position).magnitude();
        data.playerVisible = (data.playerDistance < 40.0f);

        switch (data.currentState) {
        case AIState::PATROL:
            Patrol(deltaTime, Etransform->position);
...
void Enemy::updateEnemyPosition(float deltaTime, point3d& heroPosition, point3d& enemyPositions, float& player) {
    enemyAI.AiUpdate(deltaTime, player);
