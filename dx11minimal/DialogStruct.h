...
void initContentData() {
    ...
    Dialog d2("Zenith\n\n You don't need to understand now. Just know that you were created for a great purpose, \n and in time all will become clear to you.\"",
    ...
    Dialog d4("Zenith\n\n The domain of Aries is not far from us,\n and you must find and save it from the corruption that has consumed its being. \n He occupies a special place in the cosmic order \n and it is his absence that causes ripples in the fabric of reality.",
...
void selectVariant(int variantIndex) {
    ...
    if (!selectedAnswer.soundPath.empty()) {
        ...
        // ИСПРАВЛЕНИЕ: ГГ говорит - делаем ГГ прозрачнее
        isHeroSpeaking = false;
        isZenithSpeaking = true;
        alphaChangeStartTime = currentTime;
...
void renderContent(Constellation& c) {
    ...
    float textY = (100. / 1440) * window.height;
    ...
    if (currentNarrative < (int)narratives.size()) {
        ...
        if (!currentDialogs.empty()) {
            ...
            if (!dialogSoundPlayed && !d.soundPath.empty()) {
                ...
                // ИСПРАВЛЕНИЕ: Зенит говорит - делаем Зенита прозрачнее
                isZenithSpeaking = false;
                isHeroSpeaking = true;
                alphaChangeStartTime = currentTime;
...
void handleInput() {
    ...
    if (GetAsyncKeyState('P') & 0x8000) {
        ...
        // Сбрасываем прозрачность при выходе из диалогов
        isZenithSpeaking = false;
        isHeroSpeaking = false;
        zenithAlpha = 1.0f;
        heroAlpha = 1.0f;
