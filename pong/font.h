...
float drawLetterFX(int letter, float x, float y, float scale,int num=0)
{
    if (letter == 0)
    {
        return letter_width[letter] * scale;
    ...
    float rs = clamp(sin(timeGetTime() * .002),0,1)* 100.;

    for (int i = 0; i < font[letter]->size(); i += 2)
    {
        float x_ = font[letter]->at(i) * scale;
        float y_ = font[letter]->at(i + 1) * scale;
        ...
        if (i == 0)
        {
            MoveToEx(window.context, x + p.x, y + p.y, NULL);
        }
        else
        {
            LineTo(window.context, x+ p.x, y + p.y);
        }
    }

/*    int sz = font[letter]->size() / 2;
    srand(timeGetTime());
    int i = 2*(rand() % sz);
    int j = 2*(rand() % sz);
        MoveToEx(window.context, x + font[letter]->at(j), y + font[letter]->at(j+1), NULL);
        float xx = x + font[letter]->at(i) * scale;
        float yy = y + font[letter]->at(i + 1) * scale;
        LineTo(window.context, xx, yy);
  */  
